...
export class UserService {
    ...
    async addUser(user: User): Promise<User> {
        // waits for validation to edit fields before post request
    ...
    private async addressValidation(user: User): Promise<any> {
        // ensures user placed a space between the street number and street name
        const numAndStreetName = user.hAddress.split(' ');
        let valid = true;
    ...
    private partialMatch(data: any): boolean {
        try {// throws an error if we try to access data/compare data that isn't there
            ...
            // Invalid/no gapi geocode compatible key or no "partial_match" at position 0 of result data
            console.log(error);
            return false;
        }
    }
    ...
    async updateUserInfo(user: User): Promise<object> {
        // waits for validation to edit fields before put request
        await this.addressValidation(user);
